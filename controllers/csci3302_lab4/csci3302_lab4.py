...
# as well as an array that contains the aSngles of each ray 
...
# the numpy package.
res = [0 for x in range(21)]
resAngles = [(LIDAR_ANGLE_RANGE / LIDAR_ANGLE_BINS)*i for i in range(-10,11)]
...
while robot.step(SIM_TIMESTEP) != -1:     
    ...
    for i in range(21):
        if(lidar_sensor_readings[i] < 3):
            if(300*(pose_x + lidar_sensor_readings[i] * math.sin(pose_theta - resAngles[i])) < float('inf') and 300*(pose_y + lidar_sensor_readings[i] * math.cos(pose_theta - resAngles[i])) < float('inf')):
                res[i] = [int(300*(pose_x + lidar_sensor_readings[i] * math.sin(pose_theta - resAngles[i]))), 
                int(300*(pose_y + lidar_sensor_readings[i] * math.cos(pose_theta - resAngles[i])))]
            display.drawPixel(int(res[i][0]), int(res[i][1]))

    display.setColor(0x0000FF)
    # for i in range(21):
        # x = lidar_sensor_readings[i] * math.sin(theta_o[i])
        # y = lidar_sensor_readings[i] * math.cos(theta_o[i])
        # t_matrix = [-1,-1,1]
        
        # x_map = t_matrix[0] * math.cos(pose_theta) + t_matrix[1] * (-math.sin(pose_theta)) + (t_matrix[2] * x)
        # y_map = t_matrix[0] * math.sin(pose_theta) + t_matrix[1] * (math.cos(pose_theta))  + (t_matrix[2] * y)
        # display.drawPixel(int(x_map*300), int(y_map*300))
